  ...
  <div id="mainkanji" class="h-full w-full p-5">
   ...
   <div class="container mx-auto bg-white opacity-90 w-full lg:w-2/3 drop-shadow rounded"> 
      ...
      <h2>西暦 {CalendarData[0].curr} 年</h2>
        <RangeSlider 	on:change={(e) => { handleSliderChange('all',0,e.detail.value)}} id="all" min={1868} max={2022} bind:values={CalendarData[0].curr} pips float  first='label' last='label' step={1} pipstep='60' rest='label'/>
      </div>
